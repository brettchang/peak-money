  ...
  return (
    <section className="max-w-6xl mx-auto px-6 mb-20">
      <div className="flex items-end justify-between border-b-2 border-peak-black pb-4 mb-10">
        ...
        <Link
          to="/news"
          className="text-xs font-sans uppercase tracking-widest text-peak-darkGray hover:text-peak-black transition-colors flex items-center group"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <Link to={`/news/${featured.slug}`} className="lg:col-span-8 group cursor-pointer">
        ...
        <div className="lg:col-span-4 flex flex-col space-y-8 border-t lg:border-t-0 lg:border-l border-peak-black/10 pt-8 lg:pt-0 lg:pl-8">
          {sidebar.map((article) => (
            <Link to={`/news/${article.slug}`} key={article.id} className="group cursor-pointer">
              <article className="flex gap-4">
                {article.imageUrl && (
                  <div className="flex-shrink-0 w-24 h-24 overflow-hidden bg-peak-gray">
                    <img
                ...
                <div className="flex flex-col flex-1">
                  <span className="text-xs font-sans font-bold uppercase tracking-widest text-peak-mint bg-peak-black px-2 py-0.5 self-start mb-2">
                    {article.category}
                  </span>
                  <h4 className="text-lg font-serif leading-snug mb-1 text-peak-black group-hover:text-peak-darkGray transition-colors">
                    {article.title}
                  </h4>
